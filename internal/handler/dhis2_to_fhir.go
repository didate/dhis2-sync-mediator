...
func HandleDHIS2ToFHIR(w http.ResponseWriter, r *http.Request, cfg *config.Config, ohc *openhim.OpenHIMClient) {
	...
	openhim.RespondAccepted(w, cfg.MediatorURN, "DHIS2 to FHIR conversion started")
	...
	go func() {
		...
		for _, loc := range locations {
			orgUnits = append(orgUnits, fhir.LocationToOrgUnit(&loc))
		}
		...
		for i := 0; i < cfg.MaxWorkers; i++ {
			...
			go func() {
				...
				for j := range jobs {
					...
					mr, err := fhir.DataValueSetToMeasureReport(dvs, cfg.DHIS2SourceURL)
					if err != nil {
