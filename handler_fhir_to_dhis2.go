...
func handleFHIRToDHIS2(w http.ResponseWriter, r *http.Request, cfg *Config, ohc *OpenHIMClient) {
	...
	go func() {
		...
		jobs := make(chan MeasureReport, len(reports))
		...
		for i := 0; i < cfg.MaxWorkers; i++ {
			...
			go func() {
				defer wg.Done()
				for mr := range jobs {
					dvs, err := MeasureReportToDataValueSet(&mr)
					if err != nil {
